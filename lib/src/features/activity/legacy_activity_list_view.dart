...
class LegacyActivityListView extends StatefulWidget {
  const LegacyActivityListView({super.key, this.onToggleView});

  /// Invoked when the user taps the view-toggle control to switch back to
  /// the Threaded_Activity_View (Req 12.2). Optional so this widget remains
  /// usable standalone (e.g. in a future test) without requiring a router.
  final VoidCallback? onToggleView;

  @override
  State<LegacyActivityListView> createState() => _LegacyActivityListViewState();
}

class _LegacyActivityListViewState extends State<LegacyActivityListView> {
...
// ── Public tap-through handlers ───────────────────────────────────────────────
//
// Extracted as top-level functions (rather than private State methods) so
// ThreadedActivityScreen (Phase 2) can reuse these exact same sheets for its
// own tap-through routing without introducing any new sheet widgets — see
// design.md's "No new sheet widgets are introduced in Phase 2" decision and
// Req 15.

/// Presents the pending-email-intent detail sheet with its existing cancel
/// flow, unchanged from the pre-redesign behavior (Req 15.2).
void showPendingIntentDetail(
  BuildContext context,
  EmailIntent intent,
  ZendAppModel model,
) {
...
/// Presents the outbound payment request detail sheet (share/copy-link),
/// unchanged from the pre-redesign behavior (Req 15.3).
void showOutboundRequestDetail(
